...
item("The FFT provides an efficient tool for computing the \Red{\it discrete cyclic convolution}");
equation("\sum_{p=0}^{N-1} F_p G_{k-p},");
remark("where the vectors $F$ and $G$ have period $N$.");

item("Define the \Red{\it $N$th primitive root of unity:}");
...
item("The unnormalized \Red{\it backwards discrete Fourier transform\/} of\\
$\{F_k: k=0,\ldots,N\}$ is");

equation("f_j\doteq\sum_{k=0}^{N-1}\zeta_N^{jk} F_k\qquad j=0,\ldots,N-1.");

item("The corresponding \Red{\it forward transform is}");
...
item("The pseudospectral method requires a \Red{\it linear convolution\/}.");
//since wavenumber space is not periodic.");
...
item("The terms indexed by $s\neq 0$ are \Red{\it aliases;}
we need to remove them by ensuring that $G_{k-p+sN}=0$
whenever $s\neq 0$.");
...
item("That is, one must \Red{\it zero pad} input data vectors of length~$m$ to length $N\ge 2m-1$:");

indexedfigure("explicit",0,5,"width=22cm");

item("\Red{\it Explicit zero padding} prevents mode $m-1$ from beating with itself, wrapping around to contaminate mode~$N=0\mod N$.");
...
title("Pruned FFTs\nocite{markel71,sorensen93,franchetti09}");
...
item("Let $N=2m$. For $j=0,\ldots, 2m-1$ we want to compute");

equation("
f_j=\ds\sum_{k=0}^{2m-1}\zeta_{2m}^{j k} F_k.
");
...
=\ds\sum_{k=0}^{m-1}\zeta_m^{\ell k} \Red{F_k,}
\endl
f_{2\ell+1}
...
item("\Red{\it Hermitian convolutions} arise when the input vectors are
Fourier transforms of real data:");

equation("f_{N-k}=\conj{f_k}.");

title("Centered Convolutions");
item("For a \Red{\it centered convolution,\/} the Fourier origin ($k=0$) is centered in the domain:");
...
for large $m$, this padding scheme is often referred to as the \Red{\it 2/3
padding rule.}");
...
title("2D Pseudospectral Application");

item("We need to compute:");
equation("
\Partial{\w}{t}=-\vu\dot\grad\w
...
item("The \Red{\it ternary convolution\/} of three vectors $F$, $G$, and $H$ is");
