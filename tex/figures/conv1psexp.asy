...
for(int i=0; i<6; ++i){
  ...
  p=shift(z)*g;
  draw(pic,p);
  label(pic,"$\{0\}_{k=0}^{N-1}$",center(p),labelpen);
  p1=bottom(p)-(wx/2,0);
  ...
  p=shift(z)*wg;
  filldraw(pic,p,datapen);
  label(pic,"$\{f_n\}_{n=0}^{2N-1}$",center(p),labelpen);
  q1=top(p);
  draw(pic,p1..q1,EndArrow);
  p1=bottom(p);
  ...
  p=shift(z)*wg;
  filldraw(pic,p,datapen);
  label(pic,"$\{g_n\}_{n=0}^{2N-1}$",center(p),labelpen);
  q2=top(p);
  draw(pic,p2..q2,EndArrow);
  p2=bottom(p);
  ...
  label(pic,"$\{f_ng_n\}_{n=0}^{2N-1}$",center(p),labelpen);
  q1=top(p);
  draw(pic,p1..q1,EndArrow);
  p1=bottom(p);
  ...
  p=shift(z)*g;
  filldraw(pic,p,convpen);
  label(pic,"$\{(F*G)_k\}_{k=0}^{N-1}$",center(p),labelpen);
  q1=top(p)+(wx/2,0);
  draw(pic,p1..q1,EndArrow);
  p1=bottom(p);
  ...
  shipout("conv1psexp"+(string) i,pic);
}
