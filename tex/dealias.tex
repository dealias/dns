...
\begin{algorithm}[htp]
  ...
  \medskip
  \xu $\leftarrow$ \FFTinv{\xu}\;
  \xv $\leftarrow$ \FFTinv{\xv}\;
  \xv $\leftarrow$ \xu * \xv\;
  \xu $\leftarrow$ \FFT{\xv}\;
  \medskip
  \xv $\leftarrow$ \FFTinv{\xf}\;
  \xf $\leftarrow$ \FFTinv{\xg}\;
  \xv $\leftarrow$ \xv * \xf\;
  \xf $\leftarrow$ \FFT{\xv}\;
  
  \caption{In-place convolution of two complex vectors.}\label{cconv}
\end{algorithm}
...
Also, the operation count in the two complex multiplications
in \Eq{fft0backwardsB} can be reduced by introducing the quantities
\begin{align*}
A_k&\doteq \zeta_N^k\(\Re U_k+\conj{z} \Re U_{k-m}\),\\
B_k&\doteq i\zeta_N^k\(\Im U_k+\conj{z} \Im U_{k-m}\),
\end{align*}
where $z\doteq\zeta_3^1=\(-\half,\sqrt{3}/2\)$ so that for $k > 0$,
$w_{k,1}=A_k+B_k$ and $w_{k,-1}=\conj{A_k-B_k}$.
The operation count for these implictly padded transforms is asymptotic to
$3Km\log m$. 
...
\SetKwFunction{rcFFT}{rcfft}
\SetKwFunction{crFFT}{crfft}
...
\begin{algorithm}[htp]
  ...
  \caption{In-place convolution of two centered Hermitian vectors.}\label{conv}
\end{algorithm}
