% N vs. 2m in equation 2.2
% Fix definition of cyclic convolution?
% dashed line types
% Error bars
% Add explanation in conclusions that memory savings is achieved by not
% computing all Fourier transformed data at once.
% Describe multiple convolutions.
...
Since the computational cost for a complex-to-real and real-to-complex Fourier
transform of size~$N$ is asymptotic to $K/2 N\log_2 N$, the
Hermitian convolution $\sum_{p=k-m+1}^{m-1} f_p g_{k-p}$ requires
$\fr{9}{2}Km\log_2 m$ operations.
...
be computed as out-of-place transforms, which typically execute faster than
in-place transforms (cf. **) since they require no extra (pre-, post-, or
