...
$N=4m$, as in Section~\ref{fft0bi}.
For explicit padding, one must pad the $m$ non-negative wavenumbers with at
least $m-1$ zeros, for a total vector length of $2m-1$.
For both implicit and explicit padding, it is therefore best to choose $m$
to be a power of $2$.
...
Just as for uniconvolutions, the performance and memory benefits of
...
that a implicit $8192\times 8192$ Hermitian biconvolution requires $53\%$
...
is $12m^2+15m+1$ complex words in comparision with 
$24m^2-30m+9$ complex words for an explicit transform.
