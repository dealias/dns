...
The computation times for the two methods are compared for a
one-dimensional noncentered (Fourier origin at the first index) complex
case in Figure~\ref{timing1c} and a one-dimensional centered (Fourier
origin at the central index) Hermitian case in Figure~\ref{timing1r}.
...
Intel processor. For the noncentered case, where the origin is at the first
index, the input data length $m$ is taken to be a power of $2$ and one must
pad to $n=2m-1$ to prevent the wavenumber $m-1$ from beating with itself to
affect mode~$n$. However, since power of two transforms are the most
efficient, it is typically better to extend the padding to $n=2m$.
For the centered case, we take the input data length to be odd, say $2m-1$,
with the origin at index $m-1$. One then needs to pad to $n=3m-2$
...
argument yields the so-called $2/3$ padding rule.) For implicit padding it
is convenient to pad to $n=3m$ and use a radix $3$ subdivision at the highest
level. For explicit padding, we chose $n$ to be a power of $2$ and set 
$m$ to be the largest odd integer less than or equal to $\ceil{2n/3}$,
whereas for implicit padding, we chose $m$ itself to be a power of $2$;
thereby choosing the optimal size for each case 
...
dealiased complex noncentered 1D convolutions of two vectors of (unpadded)
length $m$.}
    \label{timing1c}
  \end{center}
\end{figure}
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \caption{Comparison of computation times for explicitly and implicitly
dealiased Hermitian centered 1D convolutions of two vectors of (unpadded)
length $2m-1$.}
    \label{timing1r}
  \end{center}
\end{figure}
