...
$2m$ implicitly padded backwards Fourier transform values of a vector
...
$3m$ implicitly padded centered backwards Fourier transform values of a vector {\sf f} of length
...
and the factor $(-1)^\ell i^r$ in \Eq{fft0biforwards} reduces to $(-1)^r$.
...
$\ell=0,\ldots, 2m-1$ and $r\in\{0,1\}$, it is convenient to scale the
backwards transform by $i^r$:
\bec
i^r u_{2\ell +r}=\sum_{k=-m}^{m-1} \z_{2m}^{\ell k} i^r\z_N^{rk} U_k,
\eec
This scaling allows us to exploit the Hermitian symmetry
$$
\z_{2m}^{\ell (2m-k)} i^r\z_N^{r(2m-k)} U_{2m-k}
= \z_{2m}^{\ell (-k)} i^r(-1)^r \z_N^{-rk} U_{-k}
=\conj{\z_{2m}^{\ell k} i^r \z_N^{rk} U_k},
$$
so that only the components corresponding to non-negative wavenumbers need
to be transformed, yielding a real-valued result.

The forwards transform is then
\be
NU_k=\sum_{r=0}^{1}i^r\zeta_N^{-rk}
...
\begin{function}[htbp]
  ...
  \KwOut{vector \xf}
  \For{$k=0$ \KwTo $m-1$}{
    $\xu[k] \leftarrow i\z_{2m}^k * \xf[k]$\;
    $\xv[k] \leftarrow i\z_{2m}^k * \xg[k]$\;
    $\xw[k] \leftarrow i\z_{2m}^k * \xh[k]$\;
  }
  \medskip
  $\xu \leftarrow \crfft\inv(\xu)$\;
  $\xv \leftarrow \crfft\inv(\xv)$\;
  $\xw \leftarrow \crfft\inv(\xw)$\;
  $\xv \leftarrow \xu * \xv * \vw$\;
  $\xu \leftarrow \rcfft(\xv)$\;
  \medskip
  ...
  \medskip
  \For{$k=0$ \KwTo $m-1$}{
    $\xf[k] \leftarrow \xf[k] + i\z_{2m}^{-k} * \xu[k]$\;
  }
  \Return f/(4m)\;
\caption{biconv(vector {\sf f}, vector~{\sf g}, vector~{\sf h}) computes
an in-place implicitly dealiased biconvolution of three complex centered
Hermitian vectors {\sf f}, {\sf g}, {\sf h}, using three temporary vectors
{\sf u}, {\sf v}, and {\sf w}, each of length~$m$.}\label{biconv}
\end{function}
