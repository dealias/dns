...
to nonperiodic (wavenumber-space) data because the FFT-based
convolution assumes cyclic input and prodces cyclic output. Typically
...
worth thinking about, because this provides useful building blocks
...
length $N\ge 2m-1$ to prevent modes with wavenumber $m-1$ from beating to
contaminating mode~$N=0\mod N$. However, since FFT sizes with small prime
...
(first) mode, corresponding to wavenumber $-m+1$. The ratio of the
...
Moreover, it is convenient to implicitly pad slightly beyond $3m-2$, to $N=3m$,
...
savings for a one-dimensional transform is marginal, but gains are expected
...
Fig.~\ref{timing1t} we show that this algorithm is competitive with
...
% LocalWords:  dealiasing dealias Hermitian fftw FFT priori
% LocalWords:  nonperiodic FFTs noncentered subtransforms pre
% LocalWords:  unpadded dealised th unnormalized jk kU kj cconv
...
% LocalWords:  Canuto xm ternary convolutions tbackwards tforwards 
% LocalWords:  qm mq fp 
