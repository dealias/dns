...
without the expense of conventional zero-padding or
phase-shift techniques. In the one-dimensional complex case, the memory
requirements are identical with the usual zero-padding technique, with the
important distinction that the additional work memory need not be contiguous
with the data. In addition to being more convenient for the user, 
for higher-dimensional convolutions, this decoupling of the data and work
arrays reduces the memory usage of in-place convolutions by a factor of two.
Generalizations for computing the hyperconvolutions that arise as Fourier
transform of arbitrary powers are described.  
Highly optimized methods for both complex and centered Hermitian
convolutions can be built on top of state-of-the-art fast Fourier transform
libraries. Vectorized methods have been implemented in the open-source package
{\tt fftw++}.
\end{abstract}
