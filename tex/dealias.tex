...
multidimensional Fourier transforms \cite{SIAM91} are typically used to
reduce the memory footprint of the required spectral convolutions.

The discrete convolution, which produces cyclic output from cyclic
...
FFT algorithm such as the award-winning FFTW \cite{FFTW} library, which
...
can be quite competitive with an explicitly padded
convolution. Both the FFTW-3.2.2 library and the convolution layer we built on
...
$N$ to be a power of $2$, with $m=\floor{(n+2)/3}$. %FIXME: should this be ceil?
...
$2(N/2+1)=6c+2$, as would be used to compute a centered Hermitian
convolution with explicit padding.

For simplicity, in this work we only document the case of practical
importance, where $m=2c$. To facilitate an in-place implementation of the
backwards transform, we store the conjugate of the transformed values for
$r=1$ in reverse order in the upper half of the $f$ vector, using the
identity (for real $u_j$)
$$
...
work arrays from being reused for the $y$-convolution for each fixed $x$. The
memory savings in our method comes precisely from this reuse of work
arrays, which in turn requires that the $y$ convolutions be computed serially.

As shown in Figure~\ref{timing2c}, the resulting implicit
2D algorithm dramatically outperforms the explicit version. For example, a
$1024\times 1024$ complex convolution runs $1.92$ times faster than
% FIXME: this seems unclear now.
an explicit convolution. The gains appear to increase with transform size: a
$8192\times 8192$ implicit convolution runs in $48\%$ of the time for an
explicit one. 

The third sentence of the quote from Steven G. Johnson on
page~\ref{quote} suggests a very reasonable optimization for
explicitly padded 2D convolutions: one can omit the backwards and
forwards Fourier transforms in the $x$ direction for $m \le y < 2m$.
...
takes 73\% the time of a conventional 2D explicit convolution for size
$1024\times 1024$ but takes $171\%$ the time of a $8192\times 8192$
convolution. Our implicitly dealised convolution is also subject to
these same losses, but the savings due to implicit padding, ignoring
high-level bit reversal, in-place subtransforms, and computing the
convolution of each column as it is transformed (and thereby perhaps
still in cache) outweigh these losses.
...
in Procedure~\ref{cconv3}. The memory usage is
$4m_xm_ym_z+2m_y m_z+2m_z$ complex words, asymptotically (as $m_x\goesto\infty$)
one-quarter of the $16m_xm_ym_z$ complex words required for by an explicit
convolution.
...
$$
The basic building blocks for this problem are the centered $x$ transform
and the Hermitian $y$ convolution.
...
that an implicit $4096\times 4096$ Hermitian biconvolution requires $43\%$
...
$3\cdot 4m(2m+1)=24m^2+12m$ complex words (when $m$ is a power of $2$) 
for an explicit biconvolution. An $4096\times 4096$ $y$-pruned
biconvolution requires $68\%$ of the computation time but the same amount
of storage as an explicitly padded biconvolution.
...
$$
The computation thus reduces to $m$
