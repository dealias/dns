% Document error stuff and zeta factorization.
% Mention autosymmetrization.
...
\begin{quotation}\label{Johnsnon}
{\it
...
\eel
The transforms described by~\Eqs{cconv1backwards} and~\hEq{cconv1forwards}
...
The factors $\zeta_N^k$ in \Eqs{cconv1backwards} and~\hEq{cconv1forwards}
can be calculated accurately with a single complex multiply, using two short
precomputed tables $H_a=\zeta_N^{as}$ and $L_b=\zeta_N^b$, where
$k=as+b$ with $s=\floor{\sqrt m}$, $a=0,1,\ldots,\ceil{m/s}-1$ and
$b=0,1,\ldots,s-1$. Since these tables require only $\O(2\sqrt{m})$ complex
words of storage, we did not include them in our estimates of memory use.
We also investigated the stable recursion described by
Buneman \cite{Buneman87}. Here, the singularity at angle $\pi/2$ was removed by
replacing $\sec{\pi/2}$ with $x=1/(10\e)$ and $\sin 4\pi$ with $5x$,
$\sin{2\pi}$ and $\sin{\pi}$ with $x$. While the resulting scheme has the 
same average accuracy as the FFT itself \cite{}, 
we found that the two-table method could be implemented more efficiently
\cite{Johnson09}.
...
To compare the normalized error for the two methods, we considered the test
case $f_k=g_k=A e^{ik}$ for $k=0,\ldots,m-1$, which is nontrivial (due to
zero padding) for a dealiased linear convolution. For the complex case the
exact solution is $H_k=\sum_{p=0}^k f_p g_{k-p}=A^2 (k+1) e^{ik}$; we
arbitrarily chose $A=\root3\of3+ i\root3\of 5$.
...
a $1024^2$ implicit complex convolution is $1.83$ times faster.
As one would expect, the gains appear to increase with transform size: a
$8192^2$ implicit convolution runs $2.04$ times faster than an explicit one. 
...
illustrated in Figure~\ref{cconv2}, while the $y$-pruned explicit convolution
runs $1.38$ times faster than a conventional 2D explicit $1024^2$ convolution
but runs $1.72$ times {\it slower} than a $8192^2$ explicitly dealiased
convolution. Our implicitly dealised convolution is also subject to these
same optimization losses, but the savings due to implicit padding, in-place
subtransforms, the neglect of high-level bit reversal, and the immediate
convolution of constant $x$ columns (while perhaps still in cache) outweigh
these losses.
...
an $xz$-pruned version running $1.18$ times faster. The memory usage of an
...
$6m_x(2m_y-1)m_z+2(m_y+1)*mz+2(mz/2+1)=12m_xm_ym_z-6m_xm_z+2m_ym_z+2mz+2$
...
Asymptotically, as $m_x\goesto\infty$, we have seen that an implicit
complex convolution requires one-half of the memory required for zero
padding in two dimensions and one-quarter in three dimensions.
required by an explicit version. In the centered Hermitian case, the memory
use is $2/3$ of the amount used for an explicit convolution in two dimensions
and $4/9$ in three dimensions.
...
available set of efficient convolution sizes to include integral powers of
$2$, a case of practical significance.
For example, Canuto \etal\ \cite[p.136]{Canuto06} argues that if only a
...
yielding a slightly higher operation count of $6Km\log_2(2m)$ than the $6K
m\log_2m$ operations required by phase-shift dealiasing. The availability
of implicit convolutions now makes this argument moot.
...
open source {\tt fftw++} package that contains highly optimized
implementations of the algorithms developed in this work.

% decoupling & reuse helps keep data within cache
