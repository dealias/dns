...
unshifted (centered) %FIXME: define?
convolution is asymptotically one-half (two-thirds)
...
communication turns is prohibitively expensive. The fact that our
...
FIXME:REForDEFN at each subdivision. Effectively one wants to perform
...
Using the $N$th primitive root of unity, $\zeta_N\doteq \exp\(2 \pi i/N\)$, 
(here $\doteq$ indicates a definition), the unnormalized backwards discrete 
...
% Add procedure build(f,u); build(g,v);
...
\begin{function}[htbp]
  ...
  $\xGk \leftarrow \conj{g[m-1]}$\;
  $\xf[m-1] \leftarrow \xf[0]$\;
  $\xg[m-1] \leftarrow \xg[0]$\;
  \For{$k=1$ \KwTo $c-1$}{
    ...
    $\xf[m-1-k] \leftarrow \xA+\xB$\;
    \medskip
    $\xA \leftarrow \zeta_N^k*\[\Re \xg[k]+\(-\half,\fr{\sqrt{3}}{2}\)*\Re \xGk\]$\;
    $\xB \leftarrow -i\zeta_N^k*\[\Im \xg[k]+\(-\half,\fr{\sqrt{3}}{2}\)*\Im \xGk\]$\;
    $\xg[k] \leftarrow \xg[k]+\xGk$\;
    $\xv[k] \leftarrow \xA-\xB$\;
    $\xGk \leftarrow \conj{\xg[m-1-k]}$\;
    $\xg[m-1-k] \leftarrow \xA+\xB$\;
  }
