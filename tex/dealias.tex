...
However, it is worthwhile to first consider the response
...
As a result, in $d$ dimensions the implicitly dealised standard convolution
$\sum_{p=0}^{m-1} f_p g_{k-p}$ asymptotically uses $1/2^{d-1}$ of the
memory required by explicit padding. When the Fourier origin is
...
Implicit padding can also be used to dealias hyperconvolutions,
...
calculate higher-order convolutions efficiently.
...
$\zeta_N^k$ in \Eqs{cconv1backwards} and~\hEq{cconv1forwards}. For the
...
\section{Arbitrary $p/q$ Padding}\label{pq}
...
listed in Procedure~\ref{cconv3}. As shown in Figure~\ref{timing3c}, an implicit
...
nonlinear partial differential equations should now be a relatively
...
% LocalWords:  parallelizability pseudocode ansi ik xW ip xz ym xy Buneman's
