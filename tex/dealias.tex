...
To a frequently asked question by FFTW %FIXME:ref?
 users who want to avoid this
...
an implicitly dealised standard convolution asymptotically uses half of the
...
\begin{function}[H]
  ...
  \SetKwData{xu}{u}
  \SetKwData{xv}{v}
  \xu $\leftarrow$ $\z_{3m}^k$ \xf\;
  \xv $\leftarrow$ $\z_{3m}^k$ \xg\;
  \medskip
...
We now show how the previous routines can be used as building blocks 
to construct efficient implicitly padded 2D convolutions.

\begin{function}[htbp]
  \For{$k=1$ \KwTo $m-1$}{
  }

\caption{cconv(matrix~{\sf f}, matrix~{\sf g}) 
uses Procedure~\ref{build} to compute
an in-place implicitly dealiased convolution of centered Hermitian matrices
{\sf f} and {\sf g} of length $m\times m$ using temporary matrices 
{\sf U} and {\sf V} of length $m\times m$ and temporary vectors
{\sf u} and {\sf v} of length $m$.
.}\label{cconv2}
\end{function}
