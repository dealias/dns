...
\begin{procedure}[htbp]
  ...
  \For{$k=1$ \KwTo $c-1$}{
    $\xA \leftarrow \zeta_{6c}^k*\[\Re \xf[k]+\(-\half,\fr{\sqrt{3}}{2}\)*\Re \xFk\]$\;
    $\xB \leftarrow -i\zeta_{6c}^k*\[\Im \xf[k]+\(-\half,\fr{\sqrt{3}}{2}\)*\Im \xFk\]$\;
    $\xf[k] \leftarrow \xf[k]+\xFk$\;
    $\xu[k] \leftarrow \xA-\xB$\;
    $\xFk \leftarrow \conj{\xf[m-1-k]}$\;
    $\xf[m-1-k] \leftarrow \xA+\xB$\;
  }
  \caption{build(vector {\sf f}, vector {\sf u}) builds the required FFT arrays
...
an in-place implicitly dealiased convolution of two centered Hermitian vectors
{\sf f} and {\sf g} of length $2c$ using two temporary vectors {\sf u} and
