...
OK, this section is really in flux.  Some points:
\begin{enumerate}
\item If we accept the decimated modes as 
  $\{e^{i k \cdot x}, k = \(2 i, 2j\)\}$, then the global interpolation
  \begin{dmath}
    \text{fine grid} \xrightarrow{\text{fft}^{-1}} x-\text{space} 
    \xrightarrow{\text{fft}} \text{decimated grid}
  \end{dmath}
  just copies the even-indexed modes from the fine grid to the coarse grid. 
  That is, we could just project/prolong by copying coincident modes.

\item For symmetric symmetrization, does this mean that we can combine
  the two grids? As in, we can just put all the modes on one oddly
  shaped grid, transferring the source, with no
  $(\text{high},\text{high})\rightarrow\text{low}$ nonlinear source
  term for odd-indexed modes?  That would be awesome, but I'm not sure 
