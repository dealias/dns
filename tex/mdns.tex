...
coarse modes than there be present. Any remaining energy is then
deposited into the fine-grid modes that coincide with coarse-grid
modes in Fourier space, which automatically conserves both $E$ and
$Z$.
...
Suppose that we send $\abs{H}S$ of energy to
the modes in $S$, 
...
\end{dmath*}
In the case that $\abs{S}=2$, we can solve this 
