...
pair[] convolve0odd(pair[] f, pair[] g, pair[] u, pair[] v)
{
  int m=f.length;
  int c=quotient(m,2);
  bool even=2c == m;
  ...
  pair fc=f[c];
  f[c]=fc+fmk;
  pair A=Zetak*(fc.x+zeta3*fmk.x);
  pair B=-I*Zetak*(fc.y+zeta3*fmk.y);
  u[c]=A-B;

  pair gc=g[c];
  g[c]=gc+gmk;
  pair C=Zetak*(gc.x+zeta3*gmk.x);
  pair D=-I*Zetak*(gc.y+zeta3*gmk.y);
  Zetak *= zetac;
  v[c]=C-D;
...
pair[] convolve0(pair[] f, pair[] g, pair[] u, pair[] v)
{
  ...
  int c=quotient(m,2);
  bool even=2c == m;
  if(!even) return convolve0odd(f,g,u,v);
